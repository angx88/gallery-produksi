...
import BoronganTab from "./tabs/BoronganTab";
import KainTab from "./tabs/KainTab";
import KirimTab from "./tabs/KirimTab";
import TarifTab from "./tabs/TarifTab";
...
const PROD_COLORS = {
  ...
  Potong: { bg: "#dbeafe", text: "#1e40af", icon: "" },
...
export default function App() {
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      setIsSaving(false);
      showToast(" Proses simpan terlalu lama. Tombol Simpan dibuka lagi.", 3500);
    }, 30000);
  ...
  const refreshDataSaatIni = React.useCallback(async () => {
    ...
    try {
      ...
      showToast(" Gagal refresh data. Coba lagi.", 3500);
    } finally {
      isRefreshingDataRef.current = false;
      setRefreshingDataUi(false);
    }
    ...
    (async () => {
      try {
        ...
        if (!cancelled) {
          ...
          showToast(" Gagal memuat sebagian data. Coba refresh aplikasi.", 4000);
        }
      }
    })();
  ...
  function confirmQtyOverLimit({ process, label, limit, alreadyQty, inputQty, mode = "input" }) {
    ...
    return window.confirm(
      ` Qty ${process} melebihi qty ${label}.\n` +
      `Batas: ${limit} pcs\n` +
...
function rateDocId(productType, model, process) {
  ...
  async function addProductionEntry() {
    ...
    if (!entryForm.orderId && entryProcessWarnsWithoutOrder(entryForm.process)) {
      const okNoOrder = window.confirm(
        ` ${entryForm.process} belum dikaitkan ke pesanan.\n\n` +
  ...
  async function addPengiriman() {
    ...
    if (unlinkedSetorEntries.length > 0) {
      ...
      const okUnlinked = window.confirm(
        ` Ada ${unlinkedSetorEntries.length} entry borongan sudah setor tapi masih Tanpa Pesanan.\n${preview}\n\n` +
    ...
    if (qcBelumSelesai.length > 0) {
      ...
      const okQc = window.confirm(
        ` QC/Pengemasan belum selesai di sistem.\n${detail}\n\n` +
  ...
  async function confirmDeleteAction() {
    ...
    try {
      ...
      showToast(" Data berhasil dihapus", 3000);
    } catch (e) {
  ...
  async function batalkanSudahGajian(nama, dari = rekapDari, sampai = rekapSampai) {
    ...
    try {
      ...
      showToast(" Status gajian dibatalkan dan kasbon dikembalikan", 3500);
      refreshKasbon();
      refreshPayroll();
    } catch (e) {
  ...
  async function tandaiSudahGajianDanSimpanHistory(nama, r, dari = rekapDari, sampai = rekapSampai, carryOver = []) {
    ...
    if (belumSetorPcsPeriode > 0) {
      konfirmasiMsg += `\n\n Masih ada ${belumSetorPcsPeriode} pcs borongan periode ini yang belum setor. Item itu belum masuk gaji sekarang dan akan masuk setelah disetor.`;
    }
    if (totalKasbon > 0) {
  ...
  async function hapusGajianHistory(id) {
    ...
    try {
      ...
      showToast(" Riwayat gajian dihapus", 2500);
      refreshGajianHistory();
    } catch (e) {
  ...
  async function shareSlipGajiAsImage(nama, r, dari = rekapDari, sampai = rekapSampai, carryOver = []) {
    ...
    try {
      ...
      a.click();
      a.remove();
      showToast(" Browser ini tidak mendukung share gambar langsung. Gambar slip diunduh sebagai PNG.", 6000);
    } catch (e) {
  ...
  return (
    <div className="mx-auto min-h-screen max-w-md" style={{ background: "linear-gradient(180deg,#fff7fb 0%,#fdf2f8 45%,#faf5ff 100%)" }}>
      ...
      <DashboardTab
        ...
        setBoronganOnlyBelumSetor={setBoronganOnlyBelumSetor}
        setProduksiOnlyBelumSelesai={setProduksiOnlyBelumSelesai}
        setKirimOnlyBelumLengkap={setKirimOnlyBelumLengkap}
        setTugasDetailModal={setTugasDetailModal}
        productionEntries={productionEntries}
        setorHistoryInRange={setorHistoryInRange}
        setorTotalsFromHistory={setorTotalsFromHistory}
      ...
      />      <PesananTab
        tab={tab}
        visiblePesananOrders={visiblePesananOrders}
        ordersBelumProduksi={ordersBelumProduksi}
        ordersPerluDicek={ordersPerluDicek}
        ordersPerluDicekIds={ordersPerluDicekIds}
        pesananOnlyNeedCheck={pesananOnlyNeedCheck}
        setPesananOnlyNeedCheck={setPesananOnlyNeedCheck}
        setNeedCheckContextId={setNeedCheckContextId}
        setSearch={setSearch}
        setTab={setTab}
        setModal={setModal}
        setProdForm={setProdForm}
        todayStr={todayStr}
        orderSmallStatus={orderSmallStatus}
        produksiByOrderId={produksiByOrderId}
        shipmentByOrderId={shipmentByOrderId}
        orderHasCompletedProduction={orderHasCompletedProduction}
        isOrderClosedForNewWork={isOrderClosedForNewWork}
        ...
        fmtQty={fmtQty}
        openPengirimanForOrder={openPengirimanForOrder}
        Card={Card}
        Button={Button}
      ...
      />      <ProduksiTab
        tab={tab}
        orders={orders}
        filteredProduksi={filteredProduksi}
        produksiOnlyBelumSelesai={produksiOnlyBelumSelesai}
        setProduksiOnlyBelumSelesai={setProduksiOnlyBelumSelesai}
        setModal={setModal}
        processQtyForOrder={processQtyForOrder}
        processQtyForOrderModel={processQtyForOrderModel}
        productionEntries={productionEntries}
        getEntrySetorTotals={getEntrySetorTotals}
        updateProduksiStatus={updateProduksiStatus}
        displayModelName={displayModelName}
      ...
      />      <BoronganTab
        ...
        productionEntries={productionEntries}
        ordersForBoronganLink={ordersForBoronganLink}
        orders={orders}
        produksiByOrderId={produksiByOrderId}
        shipmentByOrderId={shipmentByOrderId}

        boronganOnlyBelumSetor={boronganOnlyBelumSetor}
        setBoronganOnlyBelumSetor={setBoronganOnlyBelumSetor}
        boronganOnlyOverSetor={boronganOnlyOverSetor}
        setBoronganOnlyOverSetor={setBoronganOnlyOverSetor}

        getEntrySetorTotals={getEntrySetorTotals}
        displayWorkerName={displayWorkerName}
        displayModelName={displayModelName}
        displayProductTypeName={displayProductTypeName}
        normalizeWorkerNameKey={normalizeWorkerNameKey}
        normalizeModelKey={normalizeModelKey}
        normalizeProcessKey={normalizeProcessKey}
        normalizeProductTypeKey={normalizeProductTypeKey}
        sameProcess={sameProcess}
        fmtQty={fmtQty}
        formatNumber={formatNumber}
        money={money}
      ...
      />      <RekapTab
        ...
        rekapDari={rekapDari}
        rekapSampai={rekapSampai}
        handleRekapDariChange={handleRekapDariChange}
        handleRekapSampaiChange={handleRekapSampaiChange}
        resetRekapToCurrentWeek={resetRekapToCurrentWeek}
        ...
        currentSundayToSaturdayPeriod={currentSundayToSaturdayPeriod}

        workerNameOptions={workerNameOptions}
        modelNameOptions={modelNameOptions}
        canonicalByExisting={canonicalByExisting}
        normalizeWorkerNameKey={normalizeWorkerNameKey}
        displayWorkerName={displayWorkerName}
        displayModelName={displayModelName}
        displayProductTypeName={displayProductTypeName}
        ...
        showFormGajianLama={showFormGajianLama}
        setShowFormGajianLama={setShowFormGajianLama}
        formGajianLama={formGajianLama}
        setFormGajianLama={setFormGajianLama}
        saveGajianLama={saveGajianLama}
      ...
      />      <EditEntryModal
        ...
        isSaving={isSaving}
        ordersForBoronganLink={ordersForBoronganLink}
        isGeneralRateProcess={isGeneralRateProcess}
        setorTotals={setorTotals}
        displayWorkerName={displayWorkerName}
        Button={Button}
      />      {tugasDetailModal && (
