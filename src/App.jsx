...
function isOrderStatusClosedForShipment(status) {
  ...
  // "selesai" tidak ditutup — admin perlu bisa input ulang untuk koreksi
  return false;
}
...
export default function App() {
  ...
  const ordersForShipment = useMemo(() => {
    return orders
      .filter((o) => modeKoreksi || (!isOrderClosedForNewWork(o, shipmentByOrderId) && !isOrderStatusClosedForShipment(o.status)))
      .sort((a, b) => String(b.createdAt || "").localeCompare(String(a.createdAt || "")));
  }, [orders, shipmentByOrderId, modeKoreksi]);
        ...
        return (
          <div className="space-y-3 p-4">
            ...
            {(() => {
              ...
              // Filter shipments sesuai periode rekap lalu group by groupId/tanggal+customer
              ...
              // Group shipments: key = groupId kalau ada, atau tanggal__customer untuk data lama
              const groupedMap = {};
              filteredShipments.forEach((s) => {
                const gid = s.raw?.groupId || s.raw?.noteNumber || "";
                const customer = (s.customer || "").toLowerCase().trim();
                const tgl = s.tanggalKirim || "";
                const key = gid || `${tgl}__${customer}`;

                if (!groupedMap[key]) {
                  groupedMap[key] = {
                    key, tanggalKirim: tgl, customer: s.customer || "-",
                    produkSet: new Set(), totalKirim: 0, _ongkir: 0, _total: 0,
                    gid, orderIds: [],
                  };
                }
                const g = groupedMap[key];
                // Kumpulkan nama produk
                (s.items || []).forEach((it) => { if (it.nama) g.produkSet.add(it.nama); });
                if (s.produk) g.produkSet.add(s.produk);
                g.totalKirim += Number(s.totalKirim || 0);
                if (s.raw?.orderId || s.raw?.pesananId) g.orderIds.push(s.raw?.orderId || s.raw?.pesananId);

                // Ongkir dari groupTotals
                const gt = groupTotals[gid] || null;
                const dt = !gt ? (dateTotals[`${tgl}__${customer}`] || null) : null;
                const source = gt || dt || {};
                let ongkir = source.ongkir || Number(s.raw?.ongkir || s.raw?.shippingCost || 0);

                // Fallback: ongkir dari order.deliveries (input Gallery Kerudung)
                if (!ongkir) {
                  const relatedOrder = orders.find(o => o.id === (s.raw?.orderId || s.raw?.pesananId || ""));
                  if (relatedOrder) {
                    const rawOrder = relatedOrder.raw || relatedOrder;
                    const deliveries = Array.isArray(rawOrder.deliveries) ? rawOrder.deliveries : [];
                    const matchedDelivery = deliveries.find(d =>
                      (gid && (d.groupId === gid || d.noteNumber === gid)) ||
                      (!gid && (d.date === tgl || d.tanggalKirim === tgl))
                    );
                    if (matchedDelivery) ongkir = Number(matchedDelivery.ongkir || matchedDelivery.shippingCost || 0);
                  }
                }
                g._ongkir += ongkir;
              ...
              const rekapShipments = Object.values(groupedMap)
                .sort((a, b) => (a.tanggalKirim > b.tanggalKirim ? -1 : 1));

              const grandQty = rekapShipments.reduce((s, x) => s + x.totalKirim, 0);
                        ...
                        <table className="w-full text-xs" style={{ minWidth: 560 }}>
                          ...
                          <tbody>
                            {rekapShipments.map((s, idx) => {
                              const produkNama = s.produkSet && s.produkSet.size > 0
                                ? Array.from(s.produkSet).join(", ")
                                : "-";
                              return (
                                <tr key={s.key} style={{ background: idx % 2 === 0 ? "#fff" : "#eff6ff" }}>
                                  ...
                                  <td className="px-3 py-2 text-right font-semibold text-blue-700">{s.totalKirim.toLocaleString("id-ID")}</td>
                                  <td className="px-3 py-2 text-right text-orange-600">{s._ongkir > 0 ? `Rp ${s._ongkir.toLocaleString("id-ID")}` : "-"}</td>
                                  <td className="px-3 py-2 text-right font-bold text-emerald-700">{s._total > 0 ? `Rp ${s._total.toLocaleString("id-ID")}` : "-"}</td>
                                </tr>
                              );
                            })}
                          </tbody>
      ...
      {tab === "kirim" && (
        <div className="space-y-3 p-4">
          ...
          <button
            ...
            style={{ background: modeKoreksi ? "#fef3c7" : "#f8fafc", color: modeKoreksi ? "#92400e" : "#64748b", border: modeKoreksi ? "1.5px solid #f59e0b" : "1.5px solid #e2e8f0" }}
          >
            {modeKoreksi ? "⚠️ Mode Koreksi Aktif — semua order tampil (ketuk untuk nonaktifkan)" : "🔧 Mode Koreksi — input ulang untuk order selesai"}
          </button>
